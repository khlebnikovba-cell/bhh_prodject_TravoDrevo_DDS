...
- `DEFAULT_CURRENCY` - валюта сообщений без явного обозначения, по умолчанию `RUB`.
...
Лист `Library` хранит канонические названия сервисов, их алиасы и категории. Лист `DDS` создается ботом автоматически и содержит формулы `QUERY` с отдельными итогами по валютам.
